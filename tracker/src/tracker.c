...
static int event_onDelete(lua_State *L) {
  lua_getfield(L, 1, "onDelete");
  return 1;
}

static int event_onTrack(lua_State *L) {
  lua_getfield(L, 1, "onTrack");
  return 1;
}
...
LRESULT CALLBACK TrackerProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, UINT_PTR uIdSubclass, DWORD_PTR dwRefData) {
    ...
    switch(msg) {
      ...
      case WM_PAINT: {
        ...
        HPEN hpen = CreatePen(PS_DOT, 1, 0xB0B0B0);
...
LUA_CONSTRUCTOR(Tracker) {
  Widget *wp, *w;
	lua_widgetinitialize(L, &wp);
  w = lua_widgetconstructor(L, CreateWindowExW(0, L"Window", NULL, WS_CHILD | WS_CLIPSIBLINGS, 0, 0, 0, 0, wp->handle, NULL, GetModuleHandle(NULL), NULL), TTracker, wp, TrackerProc);
  return 1;
}
...
static luaL_Reg Tracker_mt[] = {
  {"__gc",      Tracker___gc},
  {NULL, NULL}
};

static luaL_Reg tracker_properties[] = {
  {NULL, NULL}
};

static luaL_Reg trackerlib[] = {
  {NULL, NULL}
};
...
{
  luaL_require(L, "ui");
  lua_registerwidget(L, &TTracker, "Tracker", Tracker_constructor, WIDGET_METHODS, Tracker_mt, FALSE, FALSE, FALSE, FALSE, FALSE);
  luaL_setrawfuncs(L, Tracker_methods);
  lua_regmodule(L, tracker);
